...
- [Choose the backend state storage](03-choose-storage.md) — managed paid Postgres (single `runs` table, upsert-per-event) on a paid always-on Starter web service; history persists indefinitely, no free-tier spin-down/expiry (paid Render plan already in place).
...
- [Specify the Render deployment and config](06-render-deployment-spec.md) — all dashboard code lives under a root `dashboard/` folder; single same-origin Docker web service (ASP.NET serves API + built Vue `dist`, no CORS) on Starter, defined by a committed `render.yaml` Blueprint with paid Postgres; secrets `FACTORY_TOKEN` (`sync:false`) + DB conn `fromDatabase`; human sets token on both Render and host `.secrets/`.
