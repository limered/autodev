    ...
    <div class="card-body">
      ...
      <div class="secondary">
        <div v-if="run.prUrl" class="secondary-row">
          ...
          <a :href="run.prUrl" target="_blank" rel="noopener" class="pr-link mono">{{ run.prUrl }}</a>
        </div>
        ...
        <div v-if="run.freezeCaptured" class="secondary-row">
          ...
          <span class="freeze-path mono" title="Local snapshot path">{{ run.freezeLocalPath || '—' }}</span>
        </div>
      </div>
    </div>
  </article>
