  ...
  <article class="queue-row">
    ...
    <div class="menu-wrap">
      <button class="trigger" @click="open = !open" @keyup.escape="open = false" aria-haspopup="listbox" :aria-expanded="open">
        {{ row.picked }} <span class="muted">· {{ stageCount(row, row.picked) }}</span> <span class="chev">▾</span>
      </button>
...
<style scoped>
.queue-row { display: flex; align-items: center; min-width: 0; gap: 0.75rem; padding: 0.75rem 1rem; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); }
...
.trigger { background: none; border: none; padding: 0; font-size: 0.85rem; color: var(--text); cursor: pointer; white-space: nowrap; }
.trigger:hover { text-decoration: underline; }
.muted { color: var(--text-muted); font-size: 0.8rem; }
.chev { color: var(--text-muted); font-size: 0.75em; }
.menu { position: absolute; top: 100%; right: 0; z-index: 50; list-style: none; margin: 0.25rem 0 0; padding: 0.25rem 0; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); min-width: 13rem; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.35); }
