...
describe('useQueueActions', () => {
  it('enqueue POSTs the issueId and reports success', async () => {
    ...
    expect(ok).toBe(true)
    expect(fetchFn).toHaveBeenCalledWith('/queue', expect.objectContaining({
      method: 'POST',
  ...
  it('reorder PATCHes the id list', async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith('/queue/order', expect.objectContaining({
      method: 'PATCH',
  ...
  it('ignores re-entrant calls while an action is in flight', async () => {
    let resolve
    const fetchFn = vi.fn(() => new Promise(r => { resolve = () => r({ ok: true, status: 200 }) }))
    const actions = useQueueActions(fetchFn)
